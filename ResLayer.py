...
class ResLayer:
    def __init__(self, activation_name, dim):
        """
        :param dim: number of neurons a layer
        :param activation_name: name of activation function of this layer
        weights: (dim * dim) weights
        bias: (dim * 1) vector
        """
        self.weights = np.random.uniform(-0.5, 0.5, (dim, dim))
        self.biases = np.random.uniform(-0.5, 0.5, (dim, 1))
        self.input = None
        ...
        self.layer_func = lambda x: x + self.activation.apply(np.dot(self.weights, x) + self.biases)
    ...
    def forward(self, input_matrix):
        ...
        :param input_matrix: (dim * batch_size)  input vector
        :return: (dim * batch_size) output vector
        """
        self.input = input_matrix
        self.batch_size = input_matrix.shape[1]
    ...
    def backward(self, next_layers_gradient, jac_test):
        """
        :param next_layers_gradient: (dim * batch_size) gradient from next layers
        :return: gradient of x of this layer - (dim * batch_size)
                 gradient of theta of this layer
                 grad_f_w - (batch_size * dim * dim) * 1
                 grad_f_b - (batch_size * dim * 1) * 1
                 grad_f_theta - (dim * ( 1 + dim)) * 1
        """
        ...
        grad_f_x_i = (np.ones((self.dim, self.batch_size))) + np.dot(np.transpose(self.weights), grad_f)
        ...
        if jac_test:
            ...
            jac_x = np.identity(self.dim) + np.multiply(self.weights, np.dot(activation_derivative_one_col, np.ones((1, self.dim))))
            # [[1 + σ'(z1) * w11, σ'(z1) * w12 ,...],
            # ...
            # [σ'(zm) * wm1, ... ]]
            # jac_x -> (output_dim, input_dim)
            jac_b = activation_derivative_one_col
            self.jac_tests_w_x_b(jac_w, jac_x, jac_b, input_one_col)
    ...
    def jac_tests_w_x_b(self, jac_w, jac_x, jac_b, input_x):
        ...
        resnet = True
        test_jac_w = JacTest(JacTest.func_by_w(self.activation_name, input_x, self.biases, resnet), self.weights, "W")
        test_jac_x = JacTest(JacTest.func_by_x(self.activation_name, self.weights, self.biases, resnet), input_x, "X")
        test_jac_b = JacTest(JacTest.func_by_b(self.activation_name, input_x, self.weights, resnet), self.biases, "b")
