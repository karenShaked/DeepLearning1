...
class Decoder(nn.Module):
    def __init__(self, input_sz, hidden_state_sz, num_layers, dropout, seq_sz, output_sz):
        ...
        self.linear_prediction = nn.Linear(hidden_state_sz, self.seq_sz)
    ...
    def forward(self, z):
        ...
        o = torch.tanh(self.linear_prediction(h_t).squeeze())
        return torch.tanh(self.linear(output)), torch.tanh(self.linear_prediction(h_t).squeeze())
