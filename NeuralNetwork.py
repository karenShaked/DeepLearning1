...
class NeuralNetwork:
    ...
    def __init__(self, data_dimension,  labels_len, layer_len, hidden_activation, loss_name, final_activation):
        ...
        self.loss_layer = Loss(loss_name, final_activation, 2, labels_len)
        self.layers = []
        for i in range(layer_len):
            # self.layers.append(Layer(hidden_activation, data_dimension + i, data_dimension + (i % (layer_len - 1))))
            self.layers.append(Layer(
                hidden_activation, data_dimension, 2))
